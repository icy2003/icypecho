...
$archivesNumber = $this->db->fetchObject($this->db->select(array('COUNT(cid)' => 'num'))
        ->from('table.contents')
        ->where('table.contents.type = ?', 'post')
        ->where('table.contents.status = ?', 'publish')
        ->where('table.contents.created < ?', time())
        ->where('table.contents.authorId = ?', $this->user->uid))->num;
$commentsNumber = $this->db->fetchObject($this->db->select(array('COUNT(coid)' => 'num'))
        ->from('table.comments')
        ...
        <div class="layui-col-md9 layui-col-lg9">
            <div class="archives">
                <div class="title-page">
                    ...
                    <p><?php _e('目前共计 <em>%s</em> 篇日志，共 <em>%s</em> 条评论，加油啊~', $archivesNumber, $commentsNumber);?>
                    </p>
                </div>
                <ul class="layui-timeline">
                    ...
                    <li class="layui-timeline-item">
                        ...
                        <div class="layui-timeline-content">
                            <h3 class="layui-timeline-title"><a
                                    href="<?php $archives->permalink();?>"><?php $archives->date();?></a></h3>
                            <?php
$year = $archives->year;
